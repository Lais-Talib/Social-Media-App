      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8'>
         ...
         <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          ...
          {
            dataArray.map((tab)=>(
              <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={` cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors  ${currentTab === tab.label ? 'bg-white font-medium text-black' : 
          ...
          {dataArray.find((item)=> item.label === currentTab ).value.map((user)=>(
            <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
                  ...
                  <div className='flex-1'>
                    <p className='fort-medium text-slate-700'>{user.full_name}</p>
                    ...
                    <div className='flex max-sm:flex-col gap-2 mt-4'>
                          ...
                          }
                          {
                            currentTab === 'Following' && (
                              <button className='w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-black active:sacle-95 transition cursor-pointer'>
                                Unfollow
                              </button>
                            )
                          }
